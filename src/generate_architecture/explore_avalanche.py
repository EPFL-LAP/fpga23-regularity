...
parser.add_argument("--wire_lookahead_weight")
...
if GNL is not None:
    benchmarks = list(sorted(["%s" % b.rsplit('.', 1)[0] for b in os.listdir("gnl_benchmarks/")\
...
if args.wire_lookahead_weight is None:
    args.wire_lookahead_weight = 0.0
...
def write_avalanche_conf():
    ...
    conf = {"base_cost" : 1,\
            ...
            "wire_lookahead_weight" : float(args.wire_lookahead_weight),\
           }
...
def call_arc_gen(grid_w, grid_h, first_clique_iter = False, use_default = False):
    ...
    #Default parameters:
    in_arc_file = "grid_example/agilex/agilex.xml"
    arc_file = "agilex.xml"
    wire_file = "grid_example/agilex/agilex.wire"
    padding_file = "grid_example/agilex/agilex_padding.log"
    ...
    if args.load_cost_scaling_info is not None:
        arc_gen_switches.append("--load_cost_scaling_info %s" % args.load_cost_scaling_info)
...
def call_vpr(arc, circ, seed = None):
    ...
    run_req_files = ["%s.xml" % arc,\
                     ...
                     "base_costs.dump",\
                     "%s%s.blif" % ("gnl_benchmarks/" if GNL else '', circ),\
                     "%s.sdc" % circ\
                    ]
...
if START_FROM_ITER is not None:
    ...
    os.system("cp ilp_iter_%d.log ilp_stdout.log" % icnt)
    init = False

    with open("ilp_iter_%d.log" % icnt, "r") as inf:
        lines = inf.readlines()
    selected = [line.split()[0] for line in lines if line.split()[1] != "adopted"]
...
while not done:
    ...
    if icnt > 1 and not done:
        index_dict = {}
        with open("index.log", "r") as inf:
            lines = inf.readlines()
        for line in lines:
            switch, ind = line.split()
            index_dict.update({switch : ind})

        selected_ind = set()
        for switch in selected:
            ind = index_dict.get(switch, None)
            if ind is not None:
                selected_ind.add(ind)
            else:
                print "Missing", switch

        print len(selected_ind), len(selected)
        ...
        for line in lines[3:]:
            switch = line.split()[0]
            if not switch in selected_ind:
                txt += line
                continue
            cost = line.split()[1]
            line = line.replace(cost, "%f" % (float(cost) * cost_drop))
            txt += line
    ...
    if not done:
        #Solve the ILP
        os.system("python ilp_setup.py --vpr_log vpr_stdout.log")
        os.system("cp ilp_stdout.log ilp_iter_%d.log" % icnt)
        with open("ilp_iter_%d.log" % icnt, "r") as inf:
            lines = inf.readlines()
        if lines[0].startswith("Converged"):
            done = True
        else:
            selected = [line.split()[0] for line in lines if line.split()[1] != "adopted"]
