...
load_cost_scaling_info : Optional[str], default = None
    Specifies the file from which to load per-switch cost scaling information.
    Useful for e.g., scaling costs according to the probability of the switch participating in an ILP solution.
...
parser.add_argument("--load_cost_scaling_info")
...
##########################################################################
def read_sb_pattern_from_vpr(vpr_log_filename, rr_graph_filename, ilp_log_filename = None):
    ...
    ilp_log_filename : Optional[str], default = None
        Name of the ILP log file in case a construction ILP was solved prior
    ...
    if ilp_log_filename is not None:
        with open(ilp_log_filename, "r") as inf:
            lines = inf.readlines()

        ilp_switches = {}
        ilp_adopted = []
        for line in lines:
            switch, usage = line.split()
            if usage == "adopted":
                ilp_adopted.append(switch)
            else:
                ilp_switches.update({switch : int(usage)})

        if set(ilp_adopted) != set(stored_edges):
            print "ILP and logged adopted edges differ."
            print "ILP", sorted(ilp_adopted)
            print "Stored", sorted(stored_edges)
            raise ValueError
    ...
    if ilp_log_filename is not None:
        try:
            usage_thr = float(max(ilp_switches.values())) / ADOPTION_THRESHOLD
        except:
            usage_thr = 0.0
            global FINALIZE_CLIQUE
            FINALIZE_CLIQUE = True
            print "V1(ILP): No new switches added to the pattern. Finalizing the clique."
   
        used_potential_edges = [switch for switch, usage in ilp_switches.items() if usage >= usage_thr]
        print "Before ILP", free
        if free:
            free = [switch for switch in ilp_switches if switch in free]
        print "After ILP", free
        if free:
            used_potential_edges = free
    ...
    if args.load_pattern_from_file is not None:
        ...
        if FIRST_CLIQUE_ITER:
            ...
            existing_edges, banned_edges = read_sb_pattern_from_vpr("vpr_stdout.log",\
                                                                    ...
                                                                    "ilp_stdout.log" if not NO_STORED_UNUSED_EDGE_INCREASE else None )
...
def fill_in_template(G, cb_delay, td_sb, potential_edges = None):
    ...
    if INSERT_EMPTY_RING:
        template_filename = "minimal_template_independent_ff_empty_ring_ready.xml"
    else:
        template_filename = "minimal_template_independent_ff.xml"
...
def export_base_costs(G, filename, criticalities = None):
    ...
    if os.path.exists("avalanche.conf"):
        ...
        for line in lines:
            ...
            elif "wire_lookahead_weight" in line:
                wire_lookahead_weight = float(line.split()[1])
    ...
    txt = "%f %f %f %d %f %f\n" % (avalanche_p, avalanche_h, avalanche_d, avalanche_iter, reset_cost, wire_lookahead_weight)
    ...
    #Load per-switch cost scaling information from a file, if passed as an argument.
    cost_scaling_info = get_cost_scaling_info()
    ...
    for e in sorted(costs, key = lambda k : seg_ids[k]):
        ...
        cost *= cost_scaling_info.get(e, 1.0)
        txt += "%d %f\n" % (CHANX_START_INDEX + ecnt, cost if not GREEDY_SWITCH_SEARCH else 0.0)
        index_log += "%s %d\n" % (e, CHANX_START_INDEX + ecnt)

    with open(filename, "w") as outf:
        outf.write(txt[:-1])
    with open("index.log", "w") as outf:
        outf.write(index_log[:-1])
...
##########################################################################
def get_cost_scaling_info():
    """Reads the cost scaling information from a file.
    ...
    if args.load_cost_scaling_info is None:
        return {}

    with open(args.load_cost_scaling_info, "r") as inf:
        lines = inf.readlines()

    cost_scaling_info = {}
    for line in lines:
        switch, scale = line.split()
        cost_scaling_info.update({switch : float(scale)})

    return cost_scaling_info
##########################################################################
